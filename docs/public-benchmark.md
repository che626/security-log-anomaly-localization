...
公开数据协议、适配器、四类基线、二分类 CNN+BiGRU 路径、校准、报告器、真实 HDFS 100k 三切分实验，以及 BGL 2k 集成验证均已完成。OpenStack 的极端标签稀缺性已被审计并保留为限制。下一阶段是完成完整 BGL 和固定 Thunderbird 子集的跨系统迁移；在此之前，仓库不会把 2k 集成样本包装成完整跨系统结论。
